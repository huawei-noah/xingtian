...
from xt.model.muzero.muzero_utils_ms import value_compression_ms, value_decompression_ms, cross_entropy_ms, \
    scale_gradient_ms
...
class MuzeroModelMS(XTModel_MS):
    ...
    def __init__(self, model_info):
        ...
        self.reward_support_size = math.ceil(value_compression_ms(self.reward_max - self.reward_min)) + 1
        self.value_min = model_config.get('value_min', 0)
        self.value_max = model_config.get('value_max', 60000)
        self.value_support_size = math.ceil(value_compression_ms(self.value_max - self.value_min)) + 1
        ...
        self.net_with_loss = NetWithLoss(self.model,self.model.rnet, self.model.pnet, self.model.dnet,
                                         td_step, self.action_dim, self.weight_decay)
        self.adam = Adam(params=self.trainable_parameter, learning_rate=LR)
        self.grad_fn = ops.value_and_grad(self.net_with_loss, None, ParameterTuple(self.trainable_parameter))

        self.init_infer_net = self.InitInferNet(self.model.rnet, self.model.pnet)
        self.recur_infer_net = self.RecurInferNet(self.model.dnet, self.model.pnet)
    ...
    def recurrent_inference(self, hidden_state, action):
        ...
        value, reward, policy, hidden = self.recur_infer_net(conditioned_hidden)
    ...
    def train(self, state, label):
        ...
        obs = Tensor(state[0])
        action = Tensor(state[1], ms.int32)
        loss_weights = Tensor(state[2])
        target_value = Tensor(target_value)
        target_reward = Tensor(target_reward)
        target_policy = Tensor(label[2])
        loss = self.train_net(obs, action, loss_weights, target_value, target_reward, target_policy).asnumpy()
        return np.mean(loss)

    def get_weights(self):
        """return the weights of the model"""
        _weights = OrderedDict([(par_name, par.data.asnumpy()) for par_name, par in
                                self.model.parameters_and_names()])
        return _weights

    def set_weights(self, weights):
        """set the new weights"""
        for _, param1 in self.model.parameters_and_names():
            if param1.name in weights:
                new_param_data1 = Tensor(copy.deepcopy(weights[param1.name]))
                param1.set_data(new_param_data1, param1.sliced)
...
class NetWithLoss(nn.Cell):
    def __init__(self, full_model, representation_network, policy_network, dynamic_network, td_step, action_dim, weight_decay):
        super(NetWithLoss, self).__init__(auto_prefix=False)
        self.full_model = full_model
        self.representation_network = representation_network
        self.policy_network = policy_network
        self.dynamic_network = dynamic_network
        self.rn_params = list(self.representation_network.parameters_and_names())
        self.pn_params = list(self.policy_network.parameters_and_names())
        self.dn_params = list(self.dynamic_network.parameters_and_names())
    ...
    def construct(self, obs, action, loss_weights, target_value, target_reward, target_policy):
        hidden_state = self.representation_network(obs)
        policy_logits, value = self.policy_network(hidden_state)
        loss = cross_entropy_ms(policy_logits, target_policy[:, 0], loss_weights)
        loss += cross_entropy_ms(value, target_value[:, 0], loss_weights)
        gradient_scale = 1.0 / self.td_step
        for i in range(self.td_step):
            action_change = self.one_hot(action[:, i], self.action_dim, self.on_value, self.off_value)
            ...
            l += cross_entropy_ms(policy_logits, target_policy[:, i + 1], loss_weights)
            l += cross_entropy_ms(value, target_value[:, i + 1], loss_weights)
            loss += scale_gradient_ms(l, gradient_scale)

        for _, param in self.rn_params:
            loss += self.weight_decay * self.l2_loss(param)
        for _, param in self.dn_params:
            loss += self.weight_decay * self.l2_loss(param)
        for _, param in self.pn_params:
            loss += self.weight_decay * self.l2_loss(param)
        return loss
...
class MuzeroBaseMS(Cell):
    ...
    def __init__(self, representation_network: Cell, dynamic_network: Cell, policy_network: Cell):
        super().__init__()
        self.representation_network = representation_network
        self.dynamic_network = dynamic_network
        self.policy_network = policy_network
