...
class Evaluator(DistributedWorker):
    ...
    def _load_pretrained_model(self):
        if self.model is None:
            return
        if zeus.is_torch_backend() and self.config.pretrained_model_file is not None:
    ...
    def load_model(self):
        ...
        if not self.model_desc and not self.weights_file:
            saved_folder = self.get_local_worker_path(self.step_name, self.worker_id)
            self.weights_file = FileOps.join_path(saved_folder, 'model_{}.pth'.format(self.worker_id))
            self.model_desc = FileOps.join_path(saved_folder, 'desc_{}.json'.format(self.worker_id))
