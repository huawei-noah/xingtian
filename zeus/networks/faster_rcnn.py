...
class FasterRCNN(Module):
    ...
    def __init__(self, num_classes, backbone='ResNetDet', neck='FPN', network_name='torchvision_FasterRCNN', **kwargs):
        ...
        self.backbone = self.define_props(
            'backbone', backbone, dtype=ClassType.NETWORK)
        self.neck = self.define_props('neck', neck, dtype=ClassType.NETWORK)
        backbone_neck = Sequential(self.backbone, self.neck)
        self.head = ClassFactory.get_cls(ClassType.NETWORK, network_name)(
            backbone_neck, num_classes, **kwargs)

    def call(self, inputs, targets=None):
        """Call inputs."""
        inputs = list(image.cuda() for image in inputs)
        targets = [{k: v.cuda() for k, v in t.items()} for t in targets]
        return self.head(inputs, targets)
